...
class CRCLoss(nn.Module):
    ...
    def forward(self, features, labels=None, mask=None):        
        ...
        logits_normal = torch.index_select(logits_without_ii, 0, normal_idx)
        logits_normal_normal = torch.index_select(logits_normal, 1, normal_idx)
        logits_normal_abnormal = torch.index_select(logits_normal, 1, abnormal_idx)
...
def evaluate(normal_temp, normal_recon_temp, x_train, y_train, x_test, y_test, model, get_confidence=False, en_or_de=False):
    ...
    with torch.no_grad():
        ...
        values_features_all = values_features_all.cpu().numpy()

        values_features_test = torch.nan_to_num(F.cosine_similarity(test_features, normal_temp.reshape([-1, normal_temp.shape[0]])), nan=0.0).cpu()
        ...
        values_recon_all = values_recon_all.cpu().numpy()

        values_recon_test = torch.nan_to_num(F.cosine_similarity(test_recon, normal_recon_temp.reshape([-1, normal_recon_temp.shape[0]]), dim=1), nan=0.0).cpu()
    ...
    if mu1_fit > mu2_fit:
        gaussian1 = dist.Normal(float(mu1_fit), float(sigma1_fit), validate_args=False)
        gaussian2 = dist.Normal(float(mu2_fit), float(sigma2_fit), validate_args=False)
    else:
        gaussian2 = dist.Normal(float(mu1_fit), float(sigma1_fit), validate_args=False)
        gaussian1 = dist.Normal(float(mu2_fit), float(sigma2_fit), validate_args=False)
    ...
    if mu3_fit > mu4_fit:
        gaussian3 = dist.Normal(float(mu3_fit), float(sigma3_fit), validate_args=False)
        gaussian4 = dist.Normal(float(mu4_fit), float(sigma4_fit), validate_args=False)
    else:
        gaussian4 = dist.Normal(float(mu3_fit), float(sigma3_fit), validate_args=False)
        gaussian3 = dist.Normal(float(mu4_fit), float(sigma4_fit), validate_args=False)
